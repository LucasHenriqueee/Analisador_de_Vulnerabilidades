...
- O arquivo [IEC station 1.scd](IEC%20station%201.scd) IEC station 1.scd é um modelo real de uma subestão que utiliza o IEC 61850.
- O arquivo [SCD_model.scd](SCD_model.scd) é um arquivos derivado da SCL que foi utilizado para realizar a simulação nesse projeto. Foi por meio dele que foi elaborado a criação da topologia no GNS3.
- O arquivo [simpleIO_direct_control.cid](simpleIO_direct_control.cid) é apenas uma cofiguração final do IED dentra da IEC 61850.

Todos esses arquivos podem ser úteis para poder simular uma subestação padronizada na IEC 61850.
